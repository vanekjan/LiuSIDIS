...
class Lsidis{
 ...
 public:
  ...
  double GenerateEvent(const int mode, const int method, const int volume_flag);//Generate a sidis event
  double GetEventWeight(const int mode, const int method, const int volume_flag );//Get event weight
...
double Lsidis::GenerateEvent(const int mode = 0, const int method = 0, const int volume_flag = 1){//Generate an event and return the cross section weight
  double var[6];
  double weight = 0;
  for (int i = 0; i < 6; i++){
  ...
  if(volume_flag == 1) //default version with volume based on simualtion ranges
  {
    return weight * volume * jacobian;
  }
  else //alternative version without volume e.g. for custom binning in x, Q2 etc. Volume needs to be addressed separately
  {  
...
double Lsidis::GetEventWeight(const int mode = 0, const int method = 0, const int volume_flag = 1){//Generate an event and return the cross section weight
  double weight = 0;
  if (method == 0){//generate in x, y, z, Pt, phih, phiS
    jacobian = 2.0 * Pt;
    weight = dsigma(mode);
