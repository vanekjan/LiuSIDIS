...
const double GeV2nb = 1.0 / 389379.338;//GeV^2 times nb in natural unit
const double alpha_em = 1.0 / 137.035999074;//EM fine-structure constant
const double eu2 = 4.0 / 9.0;//up type quark charge square
const double ed2 = 1.0 / 9.0;//down type quark charge square
const double Me = 0.000510998928;//electron mass in GeV
const double Mmu = 0.1056583715;//muon mass in GeV
const double Mp = 0.938272081;//proton mass in GeV
const double Mn = 0.939565413;//neutron mass in GeV
const double MD = 1.875612945;//deuteron mass in GeV
const double MLambda = 1.115683;//Lambda mass in GeV
const double MSigmap = 1.18937;//Sigma+ mass in GeV
const double MSigmam = 1.197449;//Sigma- mass in GeV
const double Mpion = 0.13957018;//charged pion mass in GeV
const double Mpi0 = 0.1349766;//neutral pion mass in GeV
const double Mkaon = 0.493677;//charged kaon mass in GeV
const double MK0 = 0.497614;//neutral kaon mass in GeV
const double Meta = 0.77526;//rho mass in GeV
...
int Lsidis::CalculateVariables(){//calculate lorentz scalar variables
  if (st_l && st_P && st_lp && st_Ph && st_nucleus && st_hadron){
    ...
    double W2 = MD * MD + 2.0 * PP * Pq - Q2;
    if (W2 < pow(MD + Mpi0, 2)){//below the lowest threshold
      physics_control = false;
      return 0;
    }
    W = sqrt(W2);
    ...
    gamma = 2.0 * x * MD / sqrt(Q2);
    epsilon = (1.0 - y - 0.25 * gamma * gamma * y * y) / (1.0 - y + 0.5 * y * y + 0.25 * gamma * gamma * y * y);
    xn = 2.0 * x / (1.0 + sqrt(1.0 + gamma * gamma));
    double Pt2 = - (PPh * PPh) + 2.0 * (PP * PPh) * (Pq * PPh) / (PP * Pq) / (1.0 + gamma * gamma) - gamma * gamma / (1.0 + gamma * gamma) * (pow(Pq * PPh, 2) / Q2 - pow(PP * PPh, 2) / (MD * MD));
    if (Pt2 < 0) Pt2 = 0.0;
    Pt = sqrt(Pt2);
    double lt2 = - (Pl * Pl) + 2.0 * (PP * Pl) * (Pq * Pl) / (PP * Pq) / (1.0 + gamma * gamma) - gamma * gamma / (1.0 + gamma * gamma) * (pow(Pq * Pl, 2) / Q2 - pow(PP * Pl, 2) / (MD * MD));
    double ch = - 1.0 / sqrt(lt2 * Pt * Pt) * ( (Pl * PPh) - ((Pq * Pl) * (PP * PPh) + (PP * Pl) * (Pq * PPh)) / (1.0 + gamma * gamma) / (PP * Pq) + gamma * gamma / (1.0 + gamma * gamma) * ( (Pq * Pl) * (Pq * PPh) / Q2 - (PP * Pl) * (PP * PPh) / (MD * MD)));
    TMatrixD eg5(4,4);
...
int Lsidis::CalculateFinalState(){//Calculate scattered electron and detected hadron from x, y, z, Pt, phih, phiS
  if (st_l && st_P && st_nucleus && st_hadron){
    ...
    double Elp_1 = Pl_1.E() - Q2 / (2.0 * x * MD);
...
int Lsidis::CalculateRfactor(const double kT2 = 0.5, const double MiT2 = 0.5, const double MfT2 = 0.5){
  if (physics_control){
    ...
    double yh = log( (sqrt(Q2) * z * (Q2 - xn * xn * MD * MD)) / ( 2.0 * xn * xn * MD * MD * sqrt(Mh * Mh + Pt * Pt)) - sqrt(Q2) / (xn * MD) * sqrt(pow(z * (Q2 - xn * xn * MD * MD), 2) / (4.0 * xn * xn  * MD * MD * (Mh * Mh + Pt * Pt)) - 1.0));
